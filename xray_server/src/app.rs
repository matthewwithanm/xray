...
impl App {
        ...
        I: 'static + Stream<Item = IncomingMessage, Error = io::Error>
    {
        ...
        if let Some(window_updates) = inner.windows.get_mut(&window_id).unwrap().updates() {
            ...
            inner.reactor.spawn(
                receive_incoming.select(send_outgoing)
                    .then(|_: Result<((), _), ((), _)>| { // It's unclear why we need to type-annotate the closure parameter.
                        eprintln!("Done with window promise!");
                        Ok(())
                    })
            );
        } else {
